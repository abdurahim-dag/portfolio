...
SMB_BASEDIR = r'/public/Управление информационных технологий/Отдел разработки программного обеспечения/Перечень 2022 xml out'
SQL_DIR = 'SQL_DIR'
CONN_PG_ID = 'PG_GKO'
LOCAL_DIR = os.path.join(Variable.get('DATA_DIR'), 'out_manual')
...
with DAG(
        'manual-get-response-dag',
        catchup=False,
        default_args=args,
        description='Dag for load data from response manual',
        is_paused_upon_creation=True,
        start_date=pendulum.datetime(2022, 5, 5, tz="UTC"),
        schedule_interval='@once',
        tags=['response', 'manual'],
) as dag:
    ...
    t_delete = BashOperator(
        task_id='delete_task',
        bash_command=f"rm -rf {os.path.join(Variable.get('DATA_DIR'), 'out_manual')}/*",
        dag=dag
    )
    ...
    #start >> t_copy >> t_load >> t_delete >> t_update >> end
    start >> t_load >> t_delete >> t_update >> end
